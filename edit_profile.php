...
<html>
<head>
	<title> Social.ly | Edit Account Details</title>
	<meta charset="utf-8">
			...
			<form action="" method="post" enctype="multipart/form-data">
				<table class="table table-bordered table-hover">
					...
					<tr>
						...
						<td>
							<input class="form-control" type="password" name="u_recover" id="recovery_account" required value ="<?php echo $recovery_account; ?>">
							<input type="checkbox" onclick="myRecover()"><strong> Show Name</strong>
						</td>
					</tr>
					<tr align="center">
						<td colspan="6">
...
<script>
	...
	function myRecover() {
	  var x = document.getElementById("recovery_account");
	  if (x.type === "password") {
	    x.type = "text";
	  } else {
	    x.type = "password";
...
<?php
    if(isset($_POST['update'])){
        ...
        $u_recover = htmlentities($_POST['u_recover']);
        ...
        user_birthday = '$u_birthday',recovery_account = '$u_recover' where user_id = '$user_id'";
        
        $run = mysqli_query($con, $update);

        if($run){
            echo"<script>alert('Succesfully Updated Profile!!!!')</script>";
            echo"<script>window.open('edit_profile.php?u_id$userid','_self')</script>";
        } else {
